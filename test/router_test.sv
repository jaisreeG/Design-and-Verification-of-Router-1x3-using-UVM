...
class router_err_pkt_test extends router_test;

        `uvm_component_utils(router_err_pkt_test)

        router_err_pkt_vseq router_seqh;
        bit[1:0]addr;

        extern function new(string name="router_err_pkt_test",uvm_component parent);
        extern function void build_phase(uvm_phase phase);
        extern task run_phase(uvm_phase phase);

endclass

function router_err_pkt_test::new(string name="router_err_pkt_test",uvm_component parent);
        super.new(name,parent);
endfunction

function void router_err_pkt_test::build_phase(uvm_phase phase);
        super.build_phase(phase);
endfunction

task router_err_pkt_test::run_phase(uvm_phase phase);
        phase.raise_objection(this);
        repeat(50) begin
                addr={$random}%3;
                uvm_config_db#(bit[1:0])::set(this,"*","bit[1:0]",addr);
                router_seqh=router_err_pkt_vseq::type_id::create("router_seqh");
                router_seqh.start(env.v_sequencer);
        end
        phase.drop_objection(this);
endtask
