...
task router_wxtns_large_pkt ::body();
        ...
        req=write_xtn::type_id::create("req");
        start_item(req);
        assert(req.randomize() with {header[7:2] inside {[30:63]} && header[1:0]==addr;});
        `uvm_info("router_WR_SEQUENCE",$sformatf("printing from sequence \n %s",req.sprint()),UVM_HIGH)
        finish_item(req);
endtask
...
class router_wxtns_err_pkt extends router_wr_seqs;

        `uvm_object_utils(router_wxtns_err_pkt)
        bit[1:0]addr;

        extern function new(string name="router_wxtns_err_pkt");
        extern task body();

endclass

function router_wxtns_err_pkt ::new(string name ="router_wxtns_err_pkt");
        super.new(name);
endfunction

task router_wxtns_err_pkt ::body();
        if(!uvm_config_db #(bit[1:0])::get(null,get_full_name(),"bit[1:0]",addr))
                `uvm_fatal (get_type_name(),"getting the configuration failed have u set() properly")
        req=err_xtn::type_id::create("req");
        start_item(req);
